...
class FileManagerWindow:
    def __init__(self, main_app):
        ...
        self.objects = modelc_lab4.load_from_file(self.filename)
        ...
        tk.Button(frame, text="Команды", command=self.load_commands).pack(side=tk.LEFT, padx=5)
    ...
    def load_commands(self):
        ...
        cmd_file = filedialog.askopenfilename(
            title="Выберите файл с командами",
            filetypes=[("Command files", "*.cmd *.txt"), ("All files", "*.*")]
        )
        
        if not cmd_file:
            return          
        results = modelc_lab4.execute_command_file(cmd_file, self.objects)
        
        report = []
        errors = []
        for line_num, cmd, success, msg in results:
            status = "✓" if success else "✗"
            entry = f"[{line_num}] {cmd}: {status} {msg}"
            report.append(entry)
            if not success:
                errors.append(entry)
    ...
    def save(self):
        modelc_lab4.save_to_file(self.filename, self.objects)
